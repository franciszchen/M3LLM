...
output_dir = '<YOUR_OUTPUT_DIR>/PMC_Insturction_Data/stage5_compound_image'  # Save output in the same directory as input
input_filename = os.path.basename(input_file)  # Extract filename

# Extract index from filename 
file_index = re.search(r'final_instruction_compoundVQA_(\d+)\.json$', input_filename)
file_idx = file_index.group(1) if file_index else "unknown"

# Define Output Filename
output_file = os.path.join(output_dir, f"final_instruction_recontext_compoundVQA_{file_idx}.json")
